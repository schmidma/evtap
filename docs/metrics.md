...
A pair appears only after at least three samples. Bigram analysis initially shows the five fastest and five slowest qualifying pairs, including sample counts, and each list can expand to all qualifying pairs. The labels represent produced text rather than physical key positions, so the configured XKB layout affects them.
...
See the [persistence reference](persistence-spec.md) for saving, autosave, session switching, capture boundaries, and recovery behavior.
